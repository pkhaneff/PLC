...
const shuttleTaskQueueService = require('../SHUTTLE/services/shuttleTaskQueueService');
const { lifterService } = require('../../core/bootstrap');
const MissionCoordinatorService = require('../SHUTTLE/services/MissionCoordinatorService');
const mqttClientService = require('../../services/mqttClientService');
const { TASK_ACTIONS, MQTT_TOPICS } = require('../../config/shuttle.config');
const LifterCoordinationService = require('./LifterCoordinationService');
  ...
  async handleWaitingForLifter(shuttleId, eventPayload, dispatcher) {
    ...
    if (isLifterAtFloor && !isLifterBusy) {
      ...
      if (dispatcher) {
        ...
        if (taskInfo) {
          logger.info(
            `[LifterEventHandler] Immediate Resume: Resuming task ${taskInfo.taskId} for shuttle ${shuttleId}`,
          );
