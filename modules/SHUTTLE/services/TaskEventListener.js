...
const LifterEventHandler = require('../../Lifter/LifterEventHandler');
...
class TaskEventListener {
  ...
  initialize() {
    ...
    this.client.on('connect', () => {
      ...
      this.client.subscribe(this.EVENTS_TOPIC, (err) => {
        if (err) {
          logger.error(`[TaskEventListener] Failed to subscribe: ${err}`);
        } else {
  ...
  async handleEvent(message) {
    try {
      ...
      if (!taskId) {
        if (payload.taskInfo?.taskId) {
          taskId = payload.taskInfo.taskId;
        } else if (payload.meta?.taskId) {
      ...
      switch (event) {
        ...
        case 'ARRIVED_AT_LIFTER':
          if (shuttleId) {
            await LifterEventHandler.handleArrivedAtLifter(shuttleId, payload, this.dispatcher);
          }
          break;
